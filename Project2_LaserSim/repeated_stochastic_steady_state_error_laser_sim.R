...
stochastic_steady_state_error_sim = function() {
...
for(i in 1:length(costs)) {
  costs[i] = stochastic_steady_state_error_sim()
}

mean_of_costs = mean(costs)
median_of_costs = median(costs)
stdev_of_costs = sd(costs)
print(sprintf("mean: %.2f", mean_of_costs))
print(sprintf("median: %.2f", median_of_costs))
print(sprintf("sd: %.2f", stdev_of_costs))
title = sprintf("Histogram of Costs (PID Model), Iterations = %d\nmedian: %.2f, mean: %.2f, sd: %.2f", ITERATIONS, median_of_costs, mean_of_costs, stdev_of_costs)
hist(costs, col = 'blue', main = title)
