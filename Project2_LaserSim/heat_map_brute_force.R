...
kp_range = seq(-150, -110, by=1)
ki_range = seq(-65, -35, by=1)
kd_range = seq(-36, -36, by=1)
the_kd = -36
...
cost_vector = rep(0, length(kp_range) * length(ki_range))
index_into_cost_vector = 1
for(the_kp in kp_range){
  for(the_ki in ki_range){
    the_cost = simulate_laser_cooling(kp = the_kp, ki = the_ki, kd = the_kd)
    cost_vector[index_into_cost_vector] = the_cost
    index_into_cost_vector = index_into_cost_vector + 1
...
cost_matrix = matrix(data = cost_vector, nrow = length(ki_range), byrow = TRUE)
dim(cost_vector) = c(length(ki_range), length(kp_range))
cost_matrix = t(cost_vector)

colnames(cost_matrix) = ki_range
rownames(cost_matrix) = kp_range
#print(cost_matrix)

the_heat_map = heatmap(cost_matrix, Rowv = NA, Colv = NA, col = heat.colors(512),
                       main = sprintf("Cost Map for PID model\n(best kp: %3.1f, best ki: %3.1f, fixed kd: %.3.1f)\nmore red = lower cost", best_kp, best_ki, best_kd),
                       xlab = 'ki range', ylab = 'kp range')

print(sprintf("cost: %.8f", best_cost))
