...
#for P controller:
Kc = -7.895
Ti = as.integer(.Machine$integer.max)
Td = 0.0
type_of_model = '(P Model)'

# #for PI controller:
# Kc = -7.106
# Ti = 1.924 # I changed it to not have a negative sign
# Td = 0.0
# type_of_model = '(PI Model)'

#for a PID controller:
# Kc = -9.475
# Ti = 1.155 # I changed it to not have a negative sign
# Td = 0.289
...
EI[1] = 0
ED[1] = 0
CO[1] = Kc * (E[1] + (1.0 / Ti) * EI[1] * delta_t) * (1 + Td * ED[1])
...
for(i in 2:length(tt)) {
  ...
  CO[i] = Kc * (E[i] + (1.0 / Ti) * EI[i] * delta_t) * (1 + Td * ED[i])
}
# plots the process variable (laser temperature)

par(col = 'darkblue', lwd = 3)
plot(tt, PV, xlab = 'Time Ticks', ylab = 'Laser Temperature', main = paste("Process Variable over time", type_of_model, sep = " "))
lines(tt, PV)
